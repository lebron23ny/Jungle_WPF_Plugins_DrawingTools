...
namespace Jungle_WPF_Plugins_DrawingTools.DimensionToAxis
{
    ...
    class DrawingPlugin_DimToAxis : DrawingPluginBase
    {
        ...
        public override List<InputDefinition> DefineInput()
        {
            ...
            inputDefinitions.Add(TSDT.InputDefinitionFactory.CreateInputDefinition(viewBase, firstPoint_));
            inputDefinitions.Add(TSDT.InputDefinitionFactory.CreateInputDefinition(viewBase, secondPoint_));
        ...
        public override bool Run(List<InputDefinition> Input)
        {
            try
            {
                ...
                if (indexType == 0)
                {
                    myGrid.DrawPreviousAxis(dim, nameDimAttr, nameTextAttr);
                    myGrid.DrawNextAxis(dim, nameDimAttr, nameTextAttr);
                }
                else if (indexType == 1)
                {
                    myGrid.DrawPreviousAxis(dim, nameDimAttr, nameTextAttr);
